...
def search(query, silent=False):
    ...
    url = "http://{}/api/{}/search?q={}".format(DOMAIN, APIVERSION, query.replace(" ", "%20"))
    result = DataJson(url, silent=silent)
    if not silent:
        print(result)
    return result

def get_concept(id_=None, name=None, contrast_id=None, silent=False):
    '''get_concept
    ...
    base_url = "http://{}/api/{}/concept".format(DOMAIN, APIVERSION)
    parameters = {
        "id": id_,
        "name": name,
...
def get_task(id_=None, name=None, silent=False):
    '''get_task
    ...
    base_url = "http://{}/api/{}/task".format(DOMAIN, APIVERSION)
    parameters = {
        "id": id_,
        "name": name
    }
...
def get_disorder(id_=None, name=None, silent=False):
    '''get_disorder
    ...
    base_url = "http://{}/api/{}/disorder".format(DOMAIN, APIVERSION)
    parameters = {
        "id": id_,
        "name": name
    }
...
def generate_url(base_url, parameters):
    ...
    :param parameters: a dictionary with the keys being the parameter, values
     being the values of the parameter. Any values of None will not be added to
     the url.
    '''
...
def filter_concepts(concept_id=None, contrast_id=None):
    ...
    # Get concepts by concept ids
        ...
        for concept in concept_id:
            concepts = concepts + get_concept(id_=concept).json
    # Build entire tree
    else:
...
def filter_result(triples_df, filters, column_id):
    '''Apply list of filters to dataframe via the dataframe isin function
    '''
